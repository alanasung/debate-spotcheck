...
from debatesc.configs.loader import register_schema
...
def _load_stage_registry(package: str = "debatesc") -> dict:
...
def _run_stages(cfg: DictConfig, run_dir: Path) -> dict:
    ...
    if unavailable:
        ...
        print(f"      in debatesc.stages: {unavailable}")
...
def main(cfg: DictConfig) -> None:
    ...
    from debatesc.configs.loader import config_to_dict, save_resolved_config
    from debatesc.models.device import get_device
    from debatesc.utils.logging import configure_logging
    from debatesc.utils.reproducibility import set_seed
    from debatesc.utils.run_manifest import create_run_dir, save_run_metadata
