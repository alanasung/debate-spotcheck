# Design Document: Do Debate Models Learn Human Spot-Check Habits

## 1. Purpose

This document is the method contract for **Do Debate Models Learn Human Spot-Check Habits**. It specifies what is
...
One-liner: Test whether debate setups let models learn and exploit human spot-checking patterns.

Hypothesis: Test whether debate setups let models learn and exploit human spot-checking patterns.
...
|---|---|---|
| Config schema | `debatesc.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `debatesc.configs.loader` | Hydra compose API |
| Device | `debatesc.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `debatesc.models.registry` | Pinned revisions + hardware fit |
| Hooks | `debatesc.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `debatesc.models.generation` | Chat template path recording |
| Data | `debatesc.data` | Synthetic items, splits, manifests |
| Evaluation | `debatesc.evaluation` | Metrics, failure harness, protocols |
| Ablation | `debatesc.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `debatesc.reporting` | Aggregate → MD/TeX/figures |
| Cache | `debatesc.cache` | Versioned artifact store |
...
`src/debatesc/debatesc/` are preserved across regeneration.
